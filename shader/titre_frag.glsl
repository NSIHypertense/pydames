...
void main() {
    ...
    float sin_t__2 = sin(t) / 2.0;
    vec2 uv = (gl_FragCoord.xy / taille - vec2(0.5)) * (sin_t__2 / 2.0 + 1.0) * 8;
    ...
    vec2 n_5 = n / 5.0;
    float cos_t_2 = cos(t) * 2.0;
    float a = float((int(uv.x + 20.0 + t + n_5.x) + int(uv.y + 20.0 + cos_t_2 + n_5.y)) % 2) * (sin_t__2 / 25.0 + 0.02);
